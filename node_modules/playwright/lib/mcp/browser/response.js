...
class Response {
  ...
  _computRelativeTo(fileName) {
    if (this._relativeTo)
      return import_path.default.relative(this._relativeTo, fileName);
    return fileName;
  }
  async resolveFile(template, title) {
    let fileName;
    if (template.suggestedFilename)
      fileName = await this._context.outputFile(template.suggestedFilename, { origin: "llm", title });
    else
      fileName = await this._context.outputFile((0, import_utils.dateAsFileName)(template.prefix, template.ext), { origin: "code", title });
  ...
  async _build() {
    ...
    const text = renderLogChunk(tabSnapshot?.logChunk, "console", (file) => this._computRelativeTo(file));
...
function renderLogChunk(logChunk, type, relativeTo) {
  if (!logChunk)
    return [];
  const lines = [];
